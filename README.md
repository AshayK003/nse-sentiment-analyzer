...
- **SmartScore composite (0–100)** — combines recency-weighted EWMA (36h half-life), event-adjusted sentiment, headline breadth, and news volume into a single calibrated score. See [SmartScore Algorithm](docs/SMARTSCORE.md) for implementation details, formulas, and examples.
...
- **Cascade / Ripple Tracking** — when news mentions a macro driver (crude oil, rupee, gold, steel, natural gas, coal, sugar, aluminum), the dashboard now shows which other tickers are indirectly affected and why. Hand-curated relationship map covers 8 commodity groups and 27 tickers across OMCs, aviation, paints, IT, gold, steel, city gas, coal/power, logistics, sugar, and aluminum. **Per-ticker direction** — each ticker has its own sensitivity (e.g., ONGC benefits from crude rise while OMCs are hurt), so the display shows ticker-level Bullish/Bearish labels. **Ticker mention scanning** — only tickers actually mentioned in article text are shown. **Searched-ticker focus** — commodities not affecting the searched ticker are hidden. **Direction inferred from article text** — 40+ up/down keywords detect whether the commodity is rising or falling. Falls back to cautious Bearish when direction is unclear. **Commodity-specific RSS** — 3 dedicated feeds (Moneycontrol Commodities, ET Commodities, Google News Commodities) alongside 9 India market feeds, with DuckDuckGo fallback for light days. Appears on both the ticker analysis page and the home page. No extra network calls.
- **Adaptive Sentiment Engine (P0)** — Learns sentiment from actual price reactions, not labels. TF-IDF cluster learner groups headlines into semantic neighborhoods, tracks realized average price reaction per cluster, auto-adapts as market regimes shift (no retraining). Recalibrates weights every 72h against ground-truth price moves. Based on *Hybrid News Sentiment Engine* (arXiv:2606.03457).
- **News Dissemination Breadth Clustering (P0)** — Clusters related articles by content similarity (TF-IDF + DBSCAN) to measure dissemination breadth = market impact proxy. Large multi-source clusters = high-impact events. Dissemination score (0-1) shown in dashboard as confidence indicator. Based on *FinGPT* (arXiv:2412.10823).
